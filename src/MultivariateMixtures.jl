...
logpdf(fa::FactorAnalysis{T}, x::AbstractVector{T}) where {T<:Real} = logpdf(convert(MvNormal, fa), x)
logpdf(fa::FactorAnalysis{T}, X::AbstractMatrix{T}) where {T<:Real} = logpdf!(Vector{T}(undef, size(X,2)), fa, X)
logpdf!(r::AbstractArray{T}, fa::FactorAnalysis{T}, X::AbstractMatrix{T}) where {T<:Real} = logpdf!(r, convert(MvNormal, fa), X)
